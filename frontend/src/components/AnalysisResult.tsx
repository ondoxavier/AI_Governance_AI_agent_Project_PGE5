...
export function AnalysisResult({ result }: { result: Result }) {
  ...
  return <div className="space-y-5">
    ...
    {result.sections.relevant_articles.length > 0 && <section className="panel p-5"><h2 className="font-semibold">Articles et annexes pertinents</h2><ul className="list-disc pl-5">{result.sections.relevant_articles.map(x => <li key={x}>{x}</li>)}</ul></section>}
    {result.sections.obligations.length > 0 && <section className="panel p-5"><h2 className="font-semibold">Obligations applicables</h2><ul className="list-disc pl-5">{result.sections.obligations.map(x => <li key={x}>{x}</li>)}</ul></section>}
    {result.sections.jurisdiction_comparison.length > 0 && <section className="panel p-5"><h2 className="font-semibold mb-3">Comparaison EU / US / UK</h2><div className="space-y-2">{result.sections.jurisdiction_comparison.map(block => <div key={block.jurisdiction} className="flex flex-wrap items-baseline gap-2"><JurisdictionBadge value={block.jurisdiction} /><span className="badge">{block.status}</span><span>{block.statement}</span></div>)}</div></section>}
